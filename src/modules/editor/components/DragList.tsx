  ...
  return (
    ...
    >
      <div className="flex space-x-2 items-center">
        ...
        <img className=" h-7 rounded" src={item.src} />
        <p className="text-xs truncate font-medium max-w-[100px] text-zinc-200">
          {item.fileName}
        </p>
      </div>
      ...
      <div className="flex space-x-2">
        <label htmlFor={item.id + "file-input"}>
          <UploadIcon className="h-5 w-5 text-zinc-400 cursor-pointer hover:text-zinc-300 transition-all" />
        </label>
        ...
        <button onClick={() => removeImage(item.id)}>
          <XIcon className="h-5 w-5 text-zinc-400 hover:text-zinc-300 transition-all" />
        </button>
      </div>
    </Reorder.Item>
  );
